...
// ─────────────────────────────────────────────────────────────
// Technical review — error severity & rounds (manual §Etapa 7 / §10.1)
// ─────────────────────────────────────────────────────────────
export type ErrorSeverity = 'critical' | 'moderate' | 'cosmetic'
...
// Severities that block approval: §10.1 (nunca aprovar com erro crítico) and
// §Etapa 7 (moderado também exige devolução para correção antes do avanço).
export const BLOCKING_SEVERITIES: ErrorSeverity[] = ['critical', 'moderate']

// Allowed review/correction rounds before mandatory escalation to the
// coordinator (§Etapa 7 — "na 3ª devolução, escalado ao Coordenador").
...
// ─────────────────────────────────────────────────────────────
// Conference checklists (manual §Etapa 6 assinaturas / §Etapa 9 validação).
// Stored together with the visa checklist in Client.checklistDone — the item
// ids are unique-prefixed (sig_/val_) so they never collide with §7 items.
// ─────────────────────────────────────────────────────────────
export interface ConferenceGroup {
  key: 'assinaturas' | 'validacao'
  title: string
  stageKey: StageKey
  items: ChecklistItem[]
}

export const CONFERENCE_CHECKLISTS: ConferenceGroup[] = [
  {
    key: 'assinaturas',
    title: 'Conferência de assinaturas',
    stageKey: 'assinaturas',
    items: [
      { id: 'sig_passport', label: 'Assinatura idêntica ao passaporte' },
      { id: 'sig_allfields', label: 'Todos os campos assinados' },
      { id: 'sig_date', label: 'Data preenchida corretamente (MM/DD/YYYY)' },
      { id: 'sig_clean', label: 'Nenhuma rasura' },
      { id: 'sig_scanned', label: 'Páginas digitalizadas e inseridas na versão final' },
    ],
  },
  {
    key: 'validacao',
    title: 'Validação final (dupla conferência)',
    stageKey: 'validacao',
    items: [
      { id: 'val_forms_current', label: 'Todos os formulários na versão vigente' },
      { id: 'val_signatures', label: 'Assinaturas conferidas e inseridas no pacote' },
      { id: 'val_order', label: 'Pacote organizado na ordem correta' },
      { id: 'val_evidence', label: 'Evidências completas e legíveis' },
      { id: 'val_relato', label: 'Relato incluído e coerente' },
      { id: 'val_laudo', label: 'Laudo psicológico incluído (quando exigido)' },
      { id: 'val_photos', label: 'Fotos no padrão correto' },
      { id: 'val_g28', label: 'G-28 assinado pelo advogado (quando aplicável)' },
      { id: 'val_cover', label: 'Cover letter revisada' },
      { id: 'val_noblank', label: 'Nenhum campo em branco sem justificativa' },
      { id: 'val_barcode', label: 'Barcode de formulários visível e sem corte' },
      { id: 'val_copies', label: 'Cópias na quantidade exigida pelo USCIS/EOIR' },
      { id: 'val_envelope', label: 'Envelope de retorno incluído (quando exigido)' },
    ],
  },
]
